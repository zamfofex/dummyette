import {getFile, getRank} from "../variants/checkless-chess.js"
...
let fileFrom = file => move => getFile(move.from) === file
let rankFrom = rank => move => getRank(move.from) === rank
let fileTo = file => move => getFile(move.to) === file || move.rook && getFile(move.rook.from) === file
let rankTo = rank => move => getRank(move.to) === rank || move.rook && getRank(move.rook.from) === rank
let pieceFrom = type => move => move.play().at(move.to).type === type
let captures = move => Boolean(move.passing || move.before.at(move.to))
