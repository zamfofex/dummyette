import {isCheck, isCheckmate, promotionPiece, isCastling, isLowCastling, isHighCastling, capturedPiece} from "../variants/chess.js"
...
let positionTo = move =>
{
	if (!isCastling(move)) return [move.movements[0].to]
	if (Math.abs(move.movements[0].to.x - move.movements[0].from.x) === 1) return [move.movements[1].from]
	return [move.movements[0].to, move.movements[1].from]
}

let check = move => isCheck(move.play())
let checkmate = move => isCheckmate(move.play())
let promotes = move => Boolean(promotionPiece(move))
let promotesTo = type => move => promotionPiece(move)?.type === type
let fileFrom = file => move => move.movements[0].from.file === file
let rankFrom = rank => move => move.movements[0].from.rank === rank
let fileTo = file => move => positionTo(move).some(position => position.file === file)
let rankTo = rank => move => positionTo(move).some(position => position.rank === rank)
let pieceFrom = type => move => move.movements[0].piece.type === type
let captures = move => Boolean(capturedPiece(move))
let shortCastle = move => isHighCastling(move)
let longCastle = move => isLowCastling(move)
...
{
	let moves = board.moves.filter(parseMoveName(name))
	if (moves.length !== 1)
	{
		let moves = board.moves.filter(parseMoveName(name, true))
		if (moves.length === 1)
		{
			Object.freeze(moves)
			return moves
